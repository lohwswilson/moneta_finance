...
**rewrite, not a translation**: the Odoo module is faithful to Monize's and Sure's
...
Monte Carlo simulation, emergency estate access, transaction automation rules,
financial goals, real estate home equity, and FIRE metrics) while adopting
Odoo's native users/auth/i18n/currencies/reports.

## Mapping: Feature / Concept -> Odoo model

| Domain / Concept | Odoo 18 (`moneta_finance`) |
|---|---|
| Postgres RLS + GUCs (`user_id` on every table) | `ir.rule` record rules per owned model: `group_moneta_user` -> own rows, `group_moneta_manager` -> all |
...
| `transactions` + `transaction_splits` | `moneta.transaction` + `moneta.transaction.split` (with receipt image/PDF attachments, running balances, 1-click Clr toggle, payee quickfill, automated rule trigger) |
| Transaction Rule Engine | `moneta.transaction.rule` (multi-criteria matching, auto-category, clean payee, auto-tag, retroactive execution) |
| Financial Goals & Sinking Funds | `moneta.goal` & `moneta.goal.fund.wizard` (target amount, target date, required monthly savings, visual % progress) |
| Real Estate & Physical Assets | `moneta.property` (appraisal market value, mortgage linkage, net home equity $, LTV %, rental cash flow) |
...
| Budget 5-table model (budget / budget category / period / period category / alerts) | 4-model subset: `moneta.budget`, `moneta.budget.category`, `moneta.budget.period`, `moneta.budget.period.category`; payday cycle start days, visual `% Spent` progress bars |
...
| Subscription & Outflow Detector | `moneta.subscription.detector` & `moneta.subscription.detector.line` (cadence analysis, 1-click convert to recurring bills) |
...
| Executive wealth dashboard | `moneta.dashboard` (Net Worth, Cash Flow, Portfolio, Real Estate Equity, Emergency Runway, FIRE Target Progress %, Quick Action Launchpad) |

## Layout

```
odoo_addons/moneta_finance/
  models/          account, account_share, transaction(+split), transaction_rule, category, payee(+alias),
                   budget, recurring, subscription_detector, institution, investment(+price/transaction/holding/allocation),
                   gem_strategy, net_worth, dashboard, tag, loan, monte_carlo, emergency_access, property, goal, res_users
  security/        groups, ir.model.access.csv, moneta_record_rules.xml
  data/            default_categories.xml, moneta_cron.xml
  views/           per-model views + menus + dashboard + net-worth graph + gem_strategy + loan + monte_carlo + emergency + rule + goal + property + subscription_detector
  wizards/         import wizard (QIF/OFX/CSV), reconciliation wizard, goal fund wizard
...
- `18.0.3.0.0` (Sure / Maybe Finance Suite: Automated Transaction Rule Engine, Financial Goals & Sinking Funds with Kanban progress cards, Real Estate & Home Equity tracker with LTV %, FIRE & Financial Runway resilience metrics, Smart Subscription Detector, Payday Budget Cycles).
