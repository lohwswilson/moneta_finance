...
class MonetaTransaction(models.Model):
    ...
    def action_ai_enrich(self):
        ...
        from .ai_advisor import MonetaAIClient
        import json
        ...
        for tx in self:
            ...
            clean_merchant = data.get('clean_merchant')
            if clean_merchant:
                payee = self.env['moneta.payee'].search([('name', '=ilike', clean_merchant)], limit=1)
                if not payee:
                    payee = self.env['moneta.payee'].create({'name': clean_merchant})
                tx.payee_id = payee

            cat_str = data.get('category')
            if cat_str:
                matched_cat = self.env['moneta.category'].search([('name', '=ilike', cat_str)], limit=1)
                if matched_cat:
